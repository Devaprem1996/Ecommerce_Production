            ...
            {items.map((product) => (
              ...
              >
                ...
                <Link href={`/shop/${slugify(product.name)}`} className="block relative w-full aspect-square bg-neutral-100 dark:bg-neutral-850 rounded-card overflow-hidden mb-3">
                  ...
                  />
                  {product.isOrganic && (
                    <span className="absolute bottom-2.5 left-2.5 z-10 px-2 py-0.5 text-[10px] font-bold rounded bg-success/15 text-success border border-success/20 uppercase tracking-wide select-none">
                      Organic
                    </span>
                  )}
                </Link>
